 ...
 * F_CPU: 16MHz
 *
 * Created: 16.10.2017 1:08:21
 *  Author: Nikolay
 *
 * Данная библиотека была взята здесь: https://github.com/blalor/avr-softuart
 *
 */ 
...
#ifndef F_CPU
#error "F_CPU not defined in <soft_uart.h>"
#endif

#ifndef SOFT_BAUD_RATE
#error "SOFT_BAUD_RATE not defined in <soft_uart.h>"
#endif
...
#define SOFTUART_TIMERTOP ( F_CPU / SOFTUART_PRESCALE / SOFT_BAUD_RATE / 3 - 1 )
...
void soft_uart_flush_input_buffer( void );

// Tests whether an input character has been received.
unsigned char soft_uart_kbhit( void );

// Reads a character from the input buffer, waiting if necessary.
char soft_uart_getchar( void );

// To check if transmitter is busy
unsigned char soft_uart_transmit_busy( void );

// Writes a character to the serial port.
void soft_uart_putchar( const char );

// Turns on the receive function.
void soft_uart_turn_rx_on( void );

// Turns off the receive function.
void soft_uart_turn_rx_off( void );
